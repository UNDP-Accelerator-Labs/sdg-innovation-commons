...
import { commonsPlatform, page_limit } from "@/app/lib/helpers/utils";
...
export default async function Data({ id, platform, searchParams }: Props) {
  // GET THE DB SHORTKEY TO PASS TO THE API IN CASE THERE IS A PLATFORM SPECIFIC FILTER
  const db =
    commonsPlatform.find((c: any) => c.key === platform.toLowerCase())
      ?.shortkey || null;
  let databases = {};
  ...
  // Single pinboard returns object directly, not {count, data}
  const pages = Math.ceil(boardData?.total / page_limit) ?? 1;

  const platforms =
    boardData?.counts[0] === null
      ? []
      : boardData?.counts?.map((c: any) => {
  ...
  const tabs = platforms?.map((d: any) => {
    return (
      commonsPlatform.find((c: any) => c?.shortkey === d?.platform)?.key ||
      d?.platform
    );
  });
  tabs?.unshift("all");
  ...
  if (isUNDP)
    lab = `UNDP ${!country || country === "NUL" ? "Global" : country} Accelerator Lab`;
...
export async function publish(status: number, id: number) {
  const stats = status < 3 ? 3 : 1;

  const response = await fetch(`/api/pinboards/publish`, {
    method: "POST",
    headers: {
