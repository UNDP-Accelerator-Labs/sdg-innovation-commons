	...
	sections: [
		{
			...
			{ 
				...
				type: 'txt', 
				txt: 'Scanning SDG Commons data can give a sense of what grassroots innovators are working on when it comes to inventing, developing, or adopting "green" technology that helps preserve the environment and a stable climate. We started by selecting the solutions mapped on the 58 African countries (about 3,000); next, we used a Large Language Model to select those that aligned with the definition of "green/enercgy/climate tech" used by the Timbuktoo initiative. Finally, we went through about 110 of the solutions most closely aligned with that definition, and created several boards, each one gathering solutions that deployed a specific technological strategy, like "develop affordable biodigesters to make use of the abundant organic waste and turn it into energy" or "connect machines to solar panels so that they can run even in remote locations where access to energy is limited". More on the methodology can be found <a target="_blank" class="text-blue-500 cursor-pointer" href="https://github.com/UNDP-Accelerator-Labs/timbuktoo-hub-solutions/blob/main/README.md"">here</a>.' 
			},
				{
					type: 'list',
					items: [
