...
export default async function Cartouche({
	...
	return (
		...
		<div className={clsx('cartouche border-[1px] border-solid self-start', className)}>
			<div className='md:border-b-[1px] md:border-solid lg:border-b-0'>
				<img src={mapFile || ''} className='w-full mt-[-10%] lg:mt-[-20%] mb-[-30%]' />
			</div>
			<div className='flex flex-row flex-wrap gap-1.5 border-b-[1px] border-solid pb-[20px] pl-[20px] pr-[20px] md:pt-[20px] lg:pt-0 justify-center items-center'>
			...
			<div className='sdgs grid grid-cols-2 lg:flex lg:flex-col p-[20px] col-span-2 lg:col-span-1'>
				...
				{sdgs.map((d: any, i: number) => {
					const color = `sdg${d}`;
					return (
						<div key={`sdg-${i}`} className={clsx(color, 'grid grid-cols-5 gap-[20px] items-center')}>
